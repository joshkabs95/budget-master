...
class SavingsRule(models.Model):
    MODE_CHOICES = [
        ...
        ('adaptive', 'Règle adaptative glissante'),
    ]

    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='savings_rules')
    ...
    def save(self, *args, **kwargs):
        if not self.compensation_order:
            self.compensation_order = ['wants', 'needs']
